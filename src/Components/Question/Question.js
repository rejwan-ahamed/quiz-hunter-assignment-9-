...
const Question = ({ ques }) => {
  ...
  return (
    <div>
      <div
        className="eye-icon flex justify-end pr-12"
        onClick={() => setShow(!show)}
      >
        {show ? (
          <EyeSlashIcon className="h-6 w-6 text-black mt-12 cursor-pointer hover:text-blue-700" />
        ) : (
          <EyeIcon className="h-6 w-6 text-black mt-12 cursor-pointer hover:text-blue-700" />
        )}
      </div>
